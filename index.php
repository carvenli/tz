...
'NetInputSpeed2' => "$NetInputSpeed[2]", 'NetInputSpeed3' => "$NetInputSpeed[3]",
		'NetInputSpeed4' => "$NetInputSpeed[4]", 'NetInputSpeed5' => "$NetInputSpeed[5]",'NetOutSpeed2'=>"$NetOutSpeed[2]",'NetOutSpeed3'=>"$NetOutSpeed[3]",'NetOutSpeed4'=>"$NetOutSpeed[4]",'NetOutSpeed5'=>"$NetOutSpeed[5]");
...
<meta charset=utf-8">
<link rel="icon" href="favicon.ico" type="image/x-icon" />    <!-- 定义图标文件 -->
<link rel="shortcut icon" href="favicon.ico" type="image/x-icon" />   <!-- 定义收藏夹图标文件 -->
...
<div id="page">
    <div id="header">
        <h1><a href="<?php echo $phpSelf;?>"><?php echo HTTP_HOST;?></a></h1>
        <div id="download"><a href="http://www.9gan.com/9ganphp.zip" target=_blank>点此下载最新版 <?php echo $version; ?> (支持Linux,BSD,Windows)</a></div>
    </div>
...
<?php preg_match_all( "/([^\s]+):[\s]{0,}(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)/", $strs[$i], $info );?>
     <tr>
        ...
        <td width="29%">已接收 : <b><span id="NetInput<?php echo $i?>"><?php echo $NetInput[$i]?></span></b></td>
         <td width="14%">实时: <b><span id="NetInputSpeed<?php echo $i?>">0B/s</span></b></td>
        <td width="29%">已发送 : <em><span id="NetOut<?php echo $i?>"><?php echo $NetOut[$i]?></span></em></td>
        <td width="14%">实时: <em><span id="NetOutSpeed<?php echo $i?>">0B/s</span></em></td>
    </tr>
<?php endfor; ?>
</table>
<?php endif; ?>
...
?>
  ...
  <tr>
    <td>PHP Pthreads</td>
    <td><?php if((phpversion('Pthreads'))!=''){echo phpversion('Pthreads');}else{ echo "<em>×</em>";} ?></td>
    <td>Zend OPcache</td>
  ...
  <tr>
    <td>Imagick</td>
    <td><?php if((phpversion('Imagick'))!=''){echo phpversion('Imagick');}else{ echo "<em>×</em>";} ?></td>
    <td>Ioncube</td>
  ...
  <tr>
    ...
    <td>Redis</td>
    <td><?php if((phpversion('redis'))!=''){echo phpversion('redis');}else{ echo "<em>×</em>";} ?></td>
  </tr>
  <tr>
    <td>Yaml</td>
    <td><?php if((phpversion('yaml'))!=''){echo phpversion('yaml');}else{ echo "<em>×</em>";} ?></td>
    <td>PDFlib</td>
    <td><?php if((phpversion('pdflib'))!=''){echo phpversion('pdflib');}else{ echo "<em>×</em>";} ?></td>
  </tr>
</table>
...
<table width="100%" cellpadding="3" cellspacing="0" align="center">
  ...
  <tr>
    ...
    <td><?php echo isfun("pg_close"); ?>
    <?php
    if(function_exists("pg_version")) {
    	echo '&nbsp;版本：';
    	$pgconn = @pg_connect("host=localhost port=5432 dbname=postgres user=postgres password=");
    		//or die("Could not connect");
    	$pgv = pg_version($pgconn);
    	echo $pgv['client'];
    }
    ?>
    </td>
  ...
  <tr>
    <td>MongoDB 数据库：</td>
    <td><?php echo isfun("mongodb_fieldcount");?></td>
    <td>Couchdb 数据库：</td>
    <td><?php echo isfun("couchdb_close");?></td>
  </tr>
</table>
...
<table width="100%" cellpadding="3" cellspacing="0" align="center">
  ...
  <tr>
    <td width="15%">
    	地址：<input type="text" name="pghost" value="localhost" size="10" />
    </td>
    <td width="60%">
      端口：<input type="text" name="pgport" value="5432" size="10" />
      数据库：<input type="text" name="pgname" value="postgres" size="10" />
      用户名：<input type="text" name="pguser" size="10" />
      密码：<input type="password" name="pgpassword" size="10" />
    </td>
    <td width="25%">
      <input class="btn" type="submit" name="act2" value="PostgreSQL检测" />
    </td>
  </tr>
</table>
  <?php
  if ($_POST['act2'] == 'PostgreSQL检测') {
  	if(function_exists("pg_version")==1) {
  		$link = @pg_connect("host=$pghost port=$pgport dbname=$pgname user=$pguser password=$pgpassword");
  		if ($link){
...
<div id="footer">
&copy; <?php echo date("Y",time());?> <a href="http://www.9gan.com/" target=_blank>9gan.com</a> PHP探针 <?php echo $version; ?> All Rights Reserved.<br />
